...
class NotORM_Result extends NotORM_Abstract implements Iterator, ArrayAccess, Countable, JsonSerializable {
    ...
    protected function query($query, $parameters) {
        ...
        Page::Looger("QUERY EXECUTION ".$query, 2);
        $GLOBAL_QUERY_ARG[] = array_map(array($this, 'formatValue'), $parameters);
        ...
        try {
            ...
            $GLOBAL_QUERY_ERROR[] = false;
        } catch (Exception $e) {
            $GLOBAL_QUERY_ERROR[] = $e->getMessage();
        }
        $GLOBAL_QUERY_TIME[] = number_format((microtime(true) - $this->timestart) * 1000, 10);
        return $return;
    }
    ...
    function insert_multi(array $rows) {
        ...
        if (is_array($data)) {
            ...
            $insert = ($data || $this->notORM->driver == "mysql" ? "(" . implode(", ", array_keys($data)) . ") VALUES " . implode(", ", $values) : "DEFAULT VALUES");
        }
    ...
    protected function whereOperator($operator, array $args) {
        ...
        $this->where[] = (preg_match('~^\)+$~', $condition) ? $condition : ($this->where ? " $operator " : "") . "($condition)");
        return $this;
    }
    ...
    function aggregation($function) {
        ...
        if (is_array($this->query($query, $this->parameters)->fetch())) {
