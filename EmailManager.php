    ...
    public function Authenticate($username, $password, $authtype = 'LOGIN', $realm = '', $workstation = '')
    {
        ...
        switch ($authtype) {
            case 'PLAIN':
                ...
                if ($code != 235) {
                    $this->error = array('error' => 'Authentication not accepted from server',
                        'smtp_code' => $code,
                        'smtp_msg' => substr($rply, 4));
                    if ($this->do_debug >= 1) {
                        $this->edebug('SMTP -> ERROR: ' . $this->error['error'] . ': ' . $rply);
            ...
            case 'NTLM':
                ...
                if ($code != 235) {
                    $this->error = array('error' => 'Could not authenticate',
                        'smtp_code' => $code,
                        'smtp_msg' => substr($rply, 4));
                    if ($this->do_debug >= 1) {
                        $this->edebug('SMTP -> ERROR: ' . $this->error['error'] . ': ' . $rply);
            ...
            case 'CRAM-MD5':
                ...
                if ($code != 235) {
                    $this->error = array('error' => 'Credentials not accepted from server',
                        'smtp_code' => $code,
                        'smtp_msg' => substr($rply, 4));
                    if ($this->do_debug >= 1) {
                        $this->edebug('SMTP -> ERROR: ' . $this->error['error'] . ': ' . $rply);
...
class PHPMailer
{
     ...
     * If not empty, will be sent via -f to EMAIL__SendMail or as 'MAIL FROM' in smtp mode.
     * @var string
     */
    public $Sender = '';
    /**
    ...
    public $WordWrap = 0;
    /**
     * Method to send mail: ("mail", "EMAIL__SendMail", or "smtp").
     * @var string
     */
    public $Mailer = 'mail';
    /**
     * Sets the path of the EMAIL__SendMail program.
     * @var string
     */
    public $EMAIL__SendMail = '/usr/sbin/EMAIL__SendMail';
    /**
     * Determine if mail() uses a fully EMAIL__SendMail compatible MTA that
     * supports EMAIL__SendMail's "-oi -f" options
     * @var boolean
     */
    public $UseEMAIL__SendMailOptions = true;
    /**
     ...
     * @var string
     */
    public $DKIM_private = ''; //'callbackAction';
    /**
     ...
     * @access protected
     */
    protected $sign_cert_file = ''; // message only, continue processing
    /**
     * @var string The filename of a DKIM key file
     * @access protected
     */
    protected $sign_key_file = ''; // message?, likely ok to continue processing
    /**
     * @var string The password of a DKIM key
     * @access protected
     */
    protected $sign_key_pass = ''; // message, plus full stop, critical error reached
    /**
     * @var boolean Whether to throw exceptions for errors
     * @access protected
     */
    protected $exceptions = false;     // SMTP RFC specified EOL
    ...
    /**
     * Sets Mailer to send message using the $EMAIL__SendMail program.
     * @return void
     */
    public function IsEMAIL__SendMail()
    {
        if (!stristr(ini_get('EMAIL__SendMail_path'), 'EMAIL__SendMail')) {
            $this->EMAIL__SendMail = '/var/qmail/bin/EMAIL__SendMail';
        }
        $this->Mailer = 'EMAIL__SendMail';
    }
    ...
    {
        if (stristr(ini_get('EMAIL__SendMail_path'), 'qmail')) {
            $this->EMAIL__SendMail = '/var/qmail/bin/EMAIL__SendMail';
        }
        $this->Mailer = 'EMAIL__SendMail';
    }
    ...
    public function CreateHeader()
    {
        ...
        // EMAIL__SendMail and mail() extract Cc from the header before sending
        if (count($this->cc) > 0) {
            $result .= $this->AddrAppend('Cc', $this->cc);
        }

        // EMAIL__SendMail and mail() extract Bcc from the header before sending
        if ((($this->Mailer == 'EMAIL__SendMail') || ($this->Mailer == 'mail')) && (count($this->bcc) > 0)) {
    ...
    public function PostSend()
    {
        try {
            ...
            switch ($this->Mailer) {
                case 'EMAIL__SendMail':
                    return $this->EMAIL__SendMailSend($this->MIMEHeader, $this->MIMEBody);
                case 'smtp':
    ...
    /**
     * Sends mail using the $EMAIL__SendMail program.
     ...
     * @return bool
     */
    protected function EMAIL__SendMailSend($header, $body)
    {
        if ($this->Sender != '') {
            $EMAIL__SendMail = sprintf("%s -oi -f%s -t", escapeshellcmd($this->EMAIL__SendMail), escapeshellarg($this->Sender));
        } else {
            $EMAIL__SendMail = sprintf("%s -oi -t", escapeshellcmd($this->EMAIL__SendMail));
        }
        if ($this->SingleTo === true) {
            foreach ($this->SingleToArray as $val) {
                if (!@$mail = popen($EMAIL__SendMail, 'w')) {
                    throw new phpmailerException($this->Lang('execute') . $this->EMAIL__SendMail, self::STOP_CRITICAL);
                }
                fputs($mail, "To: " . $val . "\n");
                fputs($mail, $header);
                fputs($mail, $body);
    ...
    protected function MailSend($header, $body)
    {
        ...
        if ($this->Sender != '' and !ini_get('safe_mode')) {
            $old_from = ini_get('EMAIL__SendMail_from');
            ini_set('EMAIL__SendMail_from', $this->Sender);
        }
        $rt = false;
        ...
        if (isset($old_from)) {
            ini_set('EMAIL__SendMail_from', $old_from);
        }
        if (!$rt) {
     ...
     * Also, unless EMAIL__SendMail_path points to EMAIL__SendMail (or something that
     * claims to be EMAIL__SendMail), don't pass params (not a perfect fix,
     * but it will do)
     * @param string $to To
     * @param string $subject Subject
     * @param string $body Message Body
    ...
    {
        if (ini_get('safe_mode') || !($this->UseEMAIL__SendMailOptions)) {
...
class EmailSender
{
    ...
    public function __construct($host, $port, $email, $passwd, $name)
    {
        $this->HOST = $host;
        $this->PORT = $port;
        $this->EMAIL = $email;
     ...
     * @param $string Texte du mail
     * @param $destinataire Adresse email de l'utilisateur
     * @param $subject Objet du message
     * @return bool true ou false en fonction de bon déroulement de la function
     */
    public function sendEmail($string, $destinataire, $subject)
    {
        try {
            $mail = new PHPMailer();
            $mail->IsSMTP();
            ...
            $mail->From = $this->EMAIL; //adresse d’envoi correspondant au login entrée précédement
            $mail->FromName = $this->NAME; // nom qui sera affiché
            ...
            $mail->AddAttachment("");             // piéce jointe si besoin
